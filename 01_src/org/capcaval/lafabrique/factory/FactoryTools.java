...
import org.capcaval.lafabrique.factory._impl.FactoryInvocationHandler;
import org.capcaval.lafabrique.factory._impl.GenericFactoryImpl;
import org.capcaval.lafabrique.factory._impl.GenericFactoryWithPoolImpl;
...
public class FactoryTools {
	...
	public static <T> void setFactoryImplementationInstance(Class<T> objType,  T factoryInstance){
		...
		// throw an exception if no factory found
		if(fih == null){
			throw new RuntimeException("[laFabrique Factory] ERROR : The factory type : " + objType.getName() + " can not be found");
		}
		else{
	...
	public static <T, I extends T> T newFactory(Class<T>factoryType, Class<I> factoryImplType){
		...
		try {
			...
			throw new RuntimeException("[laFabrique Factory] ERROR : can not allocate the factory implementation type " + factoryImplType + " because it does not have a constructor without parameters", e);
		}
	...
	public static <T> T newImmutableFactory(Class<T>factoryType){
		if(factoryType.isInterface() == true){
			throw new RuntimeException("[laFabrique Factory] ERROR : newImmutableFactory method can not" + factoryType + " type because it is an interface" );
		}
		// allocate the factory
		T factoryImplInstance=null;
		try {
			...
			throw new RuntimeException("[laFabrique Factory] ERROR : can not allocate the factory implementation type " + factoryType + " because it does not have a constructor without parameters", e);
		}
	...
	public static <T, I extends T> GenericFactoryWithPool<T> newGenericFactoryWithPool(Class<I> FactoredType) {
		return new GenericFactoryWithPoolImpl<T>(FactoredType);
	}
}
