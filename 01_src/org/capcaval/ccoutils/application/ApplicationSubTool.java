...
import org.capcaval.ccoutils.application.ApplicationDetails.HelpDisplayDetails;
...
public class ApplicationSubTool {
	...
	static public ApplicationDetails getApplicationInformation(AppInformation info, Class<?> applicationType) {
		...
		if(asciiLogo == true){
			...
			logoDetails = new ApplicationDetails.HelpDisplayDetails(
					asciiLogoGradient, 
					asciiLogoWidthInChar);
		}
		// 

		// set it all
		ApplicationDetails appProp = new ApplicationDetails(
				appName, appVersion, appAboutStr, 
				appPropertyFile, appAuthor, 
				appAboutArray, logoDetails);
		...
		for (Field field : applicationType.getDeclaredFields()) {
			...
			for (Annotation annotation : annotationList) {
				...
				if (annotation.annotationType() == AppProperty.class) {
					...
					// add information
					propList.add(new AppPropertyInfo(
							propertyName, // name 
	...
	public static Properties loadPropertiesFromFile(String propertyFile) {
		...
		try {
			...
			System.err.println("[ccOutils Application] Error : cannot load the property file "
					+ propertyFile);
		}
	...
	static protected Object newObjectFromString(Field field, String stringValue, double min, double max, ConverterManager converterManager) {
		Class<?> type = field.getType();
	...
	static protected void checkMinAndMax(Object obj, Object obj2, double min, double max){
		...
		if (Double.isNaN(min) == false) {
			...
			if (dvalue < min) {
				throw new RuntimeException("[ccOutils Application] ERROR : the property value  " + obj + " is under the min " + min);
			}
		}
		if (Double.isNaN(max) == false) {
			double dvalue = convertObjToDouble(obj);
			if ((Double) dvalue > max) {
				throw new RuntimeException("[ccOutils Application] ERROR : the property value is " + obj + " which is above the max " + max);
			}
		}
